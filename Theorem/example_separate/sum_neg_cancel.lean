...
theorem sum_neg_cancel_from_0_to_0(n : ℕ)(hn : 1 < n)(gol:  ∑ x in range n, (-1 : ℤ ) ^ x * ↑(Nat.choose (n - 1) x) = ∑ x in range (n - 1 + 1), (-1 : ℤ ) ^ x * ↑(Nat.choose (n - 1) x)) :
   ∑ x in range n, (-1 : ℤ ) ^ x * ↑(Nat.choose (n - 1) x) = ∑ x in range (n - 1 + 1), (-1 : ℤ) ^ x * ↑(Nat.choose (n - 1) x) := by
  have zero_lt_n : 0 < n := by linarith
  apply gol
