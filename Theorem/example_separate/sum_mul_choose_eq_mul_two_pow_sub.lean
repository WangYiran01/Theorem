...
theorem sum_mul_choose_eq_mul_two_pow_sub_from_0_to_1(n : ℕ)(hn : 0 < n)(gol:  n * ∑ l in range n, Nat.choose (n - 1) l = n * 2 ^ (n - 1)) :
   ∑ k in range (n + 1), k * Nat.choose n k = n * 2 ^ (n - 1) := by
  rw[mul_choose_eq_mul_choose hn]
  rw[sum_mul_congr]
  apply gol

theorem sum_mul_choose_eq_mul_two_pow_sub_from_0_to_2(n : ℕ)(hn : 0 < n) :
   ∑ k in range (n + 1), k * Nat.choose n k = n * 2 ^ (n - 1) := by
  rw[mul_choose_eq_mul_choose hn]
  rw[sum_mul_congr]
  rw[mul_choose_two_pow hn]

theorem sum_mul_choose_eq_mul_two_pow_sub_from_1_to_1(n : ℕ)(hn : 0 < n)(gol:  n * ∑ l in range n, Nat.choose (n - 1) l = n * 2 ^ (n - 1)) :
   ∑ k in Ico 1 (n + 1), n * Nat.choose (n - 1) (k - 1) = n * 2 ^ (n - 1) := by
  rw[sum_mul_congr]
  apply gol

theorem sum_mul_choose_eq_mul_two_pow_sub_from_1_to_2(n : ℕ)(hn : 0 < n) :
   ∑ k in Ico 1 (n + 1), n * Nat.choose (n - 1) (k - 1) = n * 2 ^ (n - 1) := by
  rw[sum_mul_congr]
  rw[mul_choose_two_pow hn]
