...
theorem sum_range_succ_eq_sum_range_from_0_to_4(n : ℕ)(gol:  ∑ x in Ico 1 (n + 1), x * Nat.choose (2 * n) x = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x) :
   ∑ k in range (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in range n, Nat.choose (2 * n - 1) k := by
  rw[range_eq_Ico]
  have h01: 0 < n + 1 := by linarith
  rw [sum_eq_sum_Ico_succ_bot h01]
...
theorem sum_range_succ_eq_sum_range_from_0_to_5(n : ℕ)(gol:  ∑ k in Ico 1 (n + 1), 2 * n * Nat.choose (2 * n - 1) (k - 1) = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x) :
   ∑ k in range (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in range n, Nat.choose (2 * n - 1) k := by
  rw[range_eq_Ico]
  have h01: 0 < n + 1 := by linarith
  rw [sum_eq_sum_Ico_succ_bot h01]
...
theorem sum_range_succ_eq_sum_range_from_0_to_6(n : ℕ)(gol:  ∑ k in Ico 1 (n + 1), 2 * n * Nat.choose (2 * n - 1) (k - 1) = ∑ x in range n, 2 * n * Nat.choose (2 * n - 1) x) :
   ∑ k in range (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in range n, Nat.choose (2 * n - 1) k := by
  rw[range_eq_Ico]
  have h01: 0 < n + 1 := by linarith
  rw [sum_eq_sum_Ico_succ_bot h01]
...
theorem sum_range_succ_eq_sum_range_from_0_to_7(n : ℕ)(gol:  ∑ k in range (n + 1 - 1), 2 * n * Nat.choose (2 * n - 1) (1 + k - 1) =
...
theorem sum_range_succ_eq_sum_range_from_0_to_8(n : ℕ) :
   ∑ k in range (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in range n, Nat.choose (2 * n - 1) k := by
  rw[range_eq_Ico]
  have h01: 0 < n + 1 := by linarith
  rw [sum_eq_sum_Ico_succ_bot h01]
...
theorem sum_range_succ_eq_sum_range_from_1_to_4(n : ℕ)(gol:  ∑ x in Ico 1 (n + 1), x * Nat.choose (2 * n) x = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x) :
   ∑ k in Ico 0 (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  have h01: 0 < n + 1 := by linarith
  rw [sum_eq_sum_Ico_succ_bot h01]
  simp
...
theorem sum_range_succ_eq_sum_range_from_1_to_5(n : ℕ)(gol:  ∑ k in Ico 1 (n + 1), 2 * n * Nat.choose (2 * n - 1) (k - 1) = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x) :
   ∑ k in Ico 0 (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  have h01: 0 < n + 1 := by linarith
  rw [sum_eq_sum_Ico_succ_bot h01]
  simp
...
theorem sum_range_succ_eq_sum_range_from_1_to_6(n : ℕ)(gol:  ∑ k in Ico 1 (n + 1), 2 * n * Nat.choose (2 * n - 1) (k - 1) = ∑ x in range n, 2 * n * Nat.choose (2 * n - 1) x) :
   ∑ k in Ico 0 (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  have h01: 0 < n + 1 := by linarith
  rw [sum_eq_sum_Ico_succ_bot h01]
  simp
...
theorem sum_range_succ_eq_sum_range_from_1_to_7(n : ℕ)(gol:  ∑ k in range (n + 1 - 1), 2 * n * Nat.choose (2 * n - 1) (1 + k - 1) =
    ∑ x in range n, 2 * n * Nat.choose (2 * n - 1) x) :
   ∑ k in Ico 0 (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  have h01: 0 < n + 1 := by linarith
  rw [sum_eq_sum_Ico_succ_bot h01]
...
theorem sum_range_succ_eq_sum_range_from_1_to_8(n : ℕ) :
   ∑ k in Ico 0 (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  have h01: 0 < n + 1 := by linarith
  rw [sum_eq_sum_Ico_succ_bot h01]
  simp
...
theorem sum_range_succ_eq_sum_range_from_2_to_4(n : ℕ)(h01 : 0 < n + 1)(gol:  ∑ x in Ico 1 (n + 1), x * Nat.choose (2 * n) x = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x) :
   ∑ k in Ico 0 (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  rw [sum_eq_sum_Ico_succ_bot h01]
  simp
  have h1: ∑ k in Ico 1 (n+1), k * (choose (2 * n) k) = ∑ k in Ico 1 (n+1), (2 * n) * (choose (2*n - 1) (k-1)) := by
...
theorem sum_range_succ_eq_sum_range_from_2_to_5(n : ℕ)(h01 : 0 < n + 1)(gol:  ∑ k in Ico 1 (n + 1), 2 * n * Nat.choose (2 * n - 1) (k - 1) = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x) :
   ∑ k in Ico 0 (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  rw [sum_eq_sum_Ico_succ_bot h01]
  simp
  have h1: ∑ k in Ico 1 (n+1), k * (choose (2 * n) k) = ∑ k in Ico 1 (n+1), (2 * n) * (choose (2*n - 1) (k-1)) := by
...
theorem sum_range_succ_eq_sum_range_from_2_to_6(n : ℕ)(h01 : 0 < n + 1)(gol:  ∑ k in Ico 1 (n + 1), 2 * n * Nat.choose (2 * n - 1) (k - 1) = ∑ x in range n, 2 * n * Nat.choose (2 * n - 1) x) :
   ∑ k in Ico 0 (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  rw [sum_eq_sum_Ico_succ_bot h01]
  simp
  have h1: ∑ k in Ico 1 (n+1), k * (choose (2 * n) k) = ∑ k in Ico 1 (n+1), (2 * n) * (choose (2*n - 1) (k-1)) := by
...
theorem sum_range_succ_eq_sum_range_from_2_to_7(n : ℕ)(h01 : 0 < n + 1)(gol:  ∑ k in range (n + 1 - 1), 2 * n * Nat.choose (2 * n - 1) (1 + k - 1) =
    ∑ x in range n, 2 * n * Nat.choose (2 * n - 1) x) :
   ∑ k in Ico 0 (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  rw [sum_eq_sum_Ico_succ_bot h01]
  simp
...
theorem sum_range_succ_eq_sum_range_from_2_to_8(n : ℕ)(h01 : 0 < n + 1) :
   ∑ k in Ico 0 (n + 1), k * Nat.choose (2 * n) k = 2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  rw [sum_eq_sum_Ico_succ_bot h01]
  simp
  have h1: ∑ k in Ico 1 (n+1), k * (choose (2 * n) k) = ∑ k in Ico 1 (n+1), (2 * n) * (choose (2*n - 1) (k-1)) := by
...
theorem sum_range_succ_eq_sum_range_from_3_to_4(n : ℕ)(h01 : 0 < n + 1)(gol:  ∑ x in Ico 1 (n + 1), x * Nat.choose (2 * n) x = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x) :
   0 * Nat.choose (2 * n) 0 + ∑ k in Ico (0 + 1) (n + 1), k * Nat.choose (2 * n) k =
    2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  simp
  have h1: ∑ k in Ico 1 (n+1), k * (choose (2 * n) k) = ∑ k in Ico 1 (n+1), (2 * n) * (choose (2*n - 1) (k-1)) := by
...
theorem sum_range_succ_eq_sum_range_from_3_to_5(n : ℕ)(h01 : 0 < n + 1)(gol:  ∑ k in Ico 1 (n + 1), 2 * n * Nat.choose (2 * n - 1) (k - 1) = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x) :
   0 * Nat.choose (2 * n) 0 + ∑ k in Ico (0 + 1) (n + 1), k * Nat.choose (2 * n) k =
    2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  simp
  have h1: ∑ k in Ico 1 (n+1), k * (choose (2 * n) k) = ∑ k in Ico 1 (n+1), (2 * n) * (choose (2*n - 1) (k-1)) := by
...
theorem sum_range_succ_eq_sum_range_from_3_to_6(n : ℕ)(h01 : 0 < n + 1)(gol:  ∑ k in Ico 1 (n + 1), 2 * n * Nat.choose (2 * n - 1) (k - 1) = ∑ x in range n, 2 * n * Nat.choose (2 * n - 1) x) :
   0 * Nat.choose (2 * n) 0 + ∑ k in Ico (0 + 1) (n + 1), k * Nat.choose (2 * n) k =
    2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  simp
  have h1: ∑ k in Ico 1 (n+1), k * (choose (2 * n) k) = ∑ k in Ico 1 (n+1), (2 * n) * (choose (2*n - 1) (k-1)) := by
...
theorem sum_range_succ_eq_sum_range_from_3_to_7(n : ℕ)(h01 : 0 < n + 1)(gol:  ∑ k in range (n + 1 - 1), 2 * n * Nat.choose (2 * n - 1) (1 + k - 1) =
    ∑ x in range n, 2 * n * Nat.choose (2 * n - 1) x) :
   0 * Nat.choose (2 * n) 0 + ∑ k in Ico (0 + 1) (n + 1), k * Nat.choose (2 * n) k =
    2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  simp
...
theorem sum_range_succ_eq_sum_range_from_3_to_8(n : ℕ)(h01 : 0 < n + 1) :
   0 * Nat.choose (2 * n) 0 + ∑ k in Ico (0 + 1) (n + 1), k * Nat.choose (2 * n) k =
    2 * n * ∑ k in Ico 0 n, Nat.choose (2 * n - 1) k := by
  simp
  have h1: ∑ k in Ico 1 (n+1), k * (choose (2 * n) k) = ∑ k in Ico 1 (n+1), (2 * n) * (choose (2*n - 1) (k-1)) := by
...
theorem sum_range_succ_eq_sum_range_from_4_to_4(n : ℕ)(h01 : 0 < n + 1)(gol:  ∑ x in Ico 1 (n + 1), x * Nat.choose (2 * n) x = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x) :
...
theorem sum_range_succ_eq_sum_range_from_4_to_5(n : ℕ)(h01 : 0 < n + 1)(gol:  ∑ k in Ico 1 (n + 1), 2 * n * Nat.choose (2 * n - 1) (k - 1) = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x) :
   ∑ x in Ico 1 (n + 1), x * Nat.choose (2 * n) x = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x := by
  have h1: ∑ k in Ico 1 (n+1), k * (choose (2 * n) k) = ∑ k in Ico 1 (n+1), (2 * n) * (choose (2*n - 1) (k-1)) := by
    refine' sum_congr rfl fun y hy => _
    have h1n: y < n + 1 := by exact (mem_Ico.1 hy).2
...
theorem sum_range_succ_eq_sum_range_from_4_to_6(n : ℕ)(h01 : 0 < n + 1)(gol:  ∑ k in Ico 1 (n + 1), 2 * n * Nat.choose (2 * n - 1) (k - 1) = ∑ x in range n, 2 * n * Nat.choose (2 * n - 1) x) :
   ∑ x in Ico 1 (n + 1), x * Nat.choose (2 * n) x = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x := by
  have h1: ∑ k in Ico 1 (n+1), k * (choose (2 * n) k) = ∑ k in Ico 1 (n+1), (2 * n) * (choose (2*n - 1) (k-1)) := by
    refine' sum_congr rfl fun y hy => _
    have h1n: y < n + 1 := by exact (mem_Ico.1 hy).2
...
theorem sum_range_succ_eq_sum_range_from_4_to_7(n : ℕ)(h01 : 0 < n + 1)(gol:  ∑ k in range (n + 1 - 1), 2 * n * Nat.choose (2 * n - 1) (1 + k - 1) =
    ∑ x in range n, 2 * n * Nat.choose (2 * n - 1) x) :
   ∑ x in Ico 1 (n + 1), x * Nat.choose (2 * n) x = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x := by
  have h1: ∑ k in Ico 1 (n+1), k * (choose (2 * n) k) = ∑ k in Ico 1 (n+1), (2 * n) * (choose (2*n - 1) (k-1)) := by
    refine' sum_congr rfl fun y hy => _
...
theorem sum_range_succ_eq_sum_range_from_4_to_8(n : ℕ)(h01 : 0 < n + 1) :
   ∑ x in Ico 1 (n + 1), x * Nat.choose (2 * n) x = 2 * n * ∑ x in range n, Nat.choose (2 * n - 1) x := by
  have h1: ∑ k in Ico 1 (n+1), k * (choose (2 * n) k) = ∑ k in Ico 1 (n+1), (2 * n) * (choose (2*n - 1) (k-1)) := by
    refine' sum_congr rfl fun y hy => _
    have h1n: y < n + 1 := by exact (mem_Ico.1 hy).2
